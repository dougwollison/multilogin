...
jQuery( function( $ ) {
	...
	var $urls = $( '.multilogin-auth-url' ),
		$notice = $( '.multilogin-notice' ),
		$template = $( '#multilogin_message_template' );
	...
	function printMessage( $msg, type ) {
		...
		var message = multiloginL10n[ type ].replace( '%s', '<strong>' + domain + '</strong>' );
